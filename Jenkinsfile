pipeline{
    agent any
    environment {
        SCRIPT_PATH = '/var/jenkins_home/custom/musicplaylist'
    }
    tools {
        gradle 'gradle 8.6'
    }
    stages{
        ...
        stage('Replace Prod Properties') {
            steps {
                withCredentials([file(credentialsId: 'musicPlaylistProd', variable: 'musicPlaylistProd')]) {
                    script {
                        sh 'cp $musicPlaylistProd ./src/main/resources/application-prod.yml'
                    }
                }
            }
        }
